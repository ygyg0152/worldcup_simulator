...
function getSimQualifiers() {
  ...
  const thirdByGroup = {};
  best8.forEach(t => { thirdByGroup[t.group] = t.team_id; });
  qual.__thirdByGroup = thirdByGroup;
...
function resolveLabel(label, qual) {
  ...
  if (label.startsWith('3rd Group ')) {
    const groups = label.replace('3rd Group ', '').split('/');
    for (const g of groups) {
      if (qual.__thirdByGroup?.[g]) return qual.__thirdByGroup[g];
    }
  }
  return '0';
}
